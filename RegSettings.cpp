...
#define S_BAR_HIDDEN_VAL _T("Hidden")
...
bool CReBarSettings::Load(LPCTSTR szRegKey, LPCTSTR szPrefix, HKEY hkRootKey/* = HKEY_CURRENT_USER*/)
{
    ...
    if(err == ERROR_SUCCESS)
    {
        ...
        for(DWORD i = 0; i<m_cbBandCount;i++)
        {
            m_pBands[i].ID = 0;
            m_pBands[i].cx = 0;
            m_pBands[i].BreakLine = false;
            m_pBands[i].Hidden = false;
            ...
            DWORD dw;
            if (reg.QueryDWORDValue(s + S_BAR_BREAKLINE_VAL, dw) == ERROR_SUCCESS)
                m_pBands[i].BreakLine = dw != 0;
            if (reg.QueryDWORDValue(s + S_BAR_HIDDEN_VAL, dw) == ERROR_SUCCESS)
                m_pBands[i].Hidden = dw != 0;
        }
    }
...
bool CReBarSettings::Save(LPCTSTR szRegKey, LPCTSTR szPrefix, HKEY hkRootKey/* = HKEY_CURRENT_USER*/)const
{
    ...
    if(err == ERROR_SUCCESS)
    {
        ...
        for(DWORD i = 0; i<m_cbBandCount;i++)
        {
            ...
            reg.SetDWORDValue(s + S_BAR_BREAKLINE_VAL, m_pBands[i].BreakLine);
            reg.SetDWORDValue(s + S_BAR_HIDDEN_VAL, m_pBands[i].Hidden);
        }
    }
...
void CReBarSettings::GetFrom(CReBarCtrl& ReBar)
{
    ...
    for(UINT i = 0; i<m_cbBandCount; i++)
    {
        ...
        m_pBands[i].Hidden = (rbi.fStyle & RBBS_HIDDEN) != 0;
    }
}
